...
<BODY>
	<FORM id="customerForm" name="customerForm"
		action="${pageContext.request.contextPath }/customer_queryBy.action"
		method=post>
										...
										<TABLE id=grid
											...
											<c:forEach items="${pageBean.list }" var="customer">
												...
												<TR
														...
														<TD><s:property value="#cus.custLinkman" /></TD>
														<TD>
								...
								<TR>
									<TD><SPAN id=pagelink>
											<DIV
												...
												共[<B>${pageBean.totalCount}</B>]条记录,[<B>${pageBean.totalPage}</B>]页 ,当前第[<b>${pageBean.currentPage}</b>]页
												<c:if test="${pageBean.currentPage!=1}">
												[<A href="${pageContext.request.contextPath}/customer_pagelist.action?currentPage=${pageBean.currentPage-1}">前一页</A>] 
												</c:if>
												<c:if test="${pageBean.currentPage!=pageBean.totalPage}">
												[<A href="${pageContext.request.contextPath}/customer_pagelist.action?currentPage=${pageBean.currentPage+1}">后一页</A>]
												</c:if>
